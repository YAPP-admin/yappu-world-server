        ...
        feature("개인 팀 관리") {
            ...
            scenario("사용자 활동 내역에서 팀 추가") {
                val team = teamRepository
                    .save(
                        getTeamEntityFixture(generation = 35, name = "A팀")
                    ).also { savedTeam ->
                        serviceRepository.save(
                            getTeamServiceEntityFixture(team = savedTeam, hasApp = true)
                        )
                    }
            ...
            scenario("사용자 활동 내역에서 팀 변경") {
                val team1 = teamRepository
                    .save(
                        getTeamEntityFixture(generation = 35, name = "팀1")
                    ).also { savedTeam ->
                        serviceRepository.save(
                            getTeamServiceEntityFixture(team = savedTeam, name = "서비스1", hasApp = true)
                        )
                    }

                val team2 = teamRepository
                    .save(
                        getTeamEntityFixture(generation = 35, name = "팀2")
                    ).also { savedTeam ->
                        serviceRepository.save(
                            getTeamServiceEntityFixture(team = savedTeam, name = "서비스2", hasWeb = true)
                        )
                    }
            ...
            scenario("사용자 활동 내역에서 팀 삭제") {
                val team = teamRepository
                    .save(
                        ...
                        serviceRepository.save(
                            getTeamServiceEntityFixture(team = savedTeam, name = "테스트 서비스", hasApp = true)
                        )
                    }
                val activityUnitId = UUID.randomUUID()
                adminTeamService.assignMemberToTeam(activityUnitId, team.id)
        ...
        feature("기수로 팀 목록 조회") {
            ...
            scenario("특정 기수의 팀 목록을 조회한다") {
                teamRepository
                    .saveAll(
                        List(2) { index ->
                            getTeamEntityFixture(generation = 35, name = "35기팀${index + 1}")
                        } + List(1) {
                            getTeamEntityFixture(generation = 36, name = "36기팀")
                        }
                    ).also { teams ->
